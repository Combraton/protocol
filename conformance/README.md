...
| `start_client`, `stop_client`, `await_client_exit` | Client-only third-party implementations (M6-Q2, Unix-socket compositions): `start_client` launches `thirdparty/<implementation>/participant.json` (`role: client`) with a rendered `config` (variables `socket.<name>`, `credential.<principal>`, `clock_file`); `stop_client` ends its input and requires `exit_code` (default 0); `await_client_exit` waits for a one-shot client's exit status. A client is observed only through public operations at the providers. Fixture `client_kills` names client mutants that `check-mutants` runs with `--client-mutant IMPLEMENTATION=NAME`. See [thirdparty](thirdparty/README.md). |
...
Expectations are `{"ok": pattern}`, `{"error": code, "details": pattern}`, or `{"any_of": [expectation, …]}` for behaviors the spec leaves to the provider. Object patterns match subsets of members. Array patterns match exactly, in length and order; use `$contains` for membership. Patterns support `$var`, `$ne_var`, `$type`, `$contains`, `$len`, `$absent`, `$any`, `$exact`, `$sha256_base64` (the sha256 digest of the decoded bytes matches the argument pattern, typically a captured digest), `$canonical_sha256` (the sha256 digest of the value's canonical encoding matches the argument pattern, so a reader recomputes a record digest) and `$base64_json` (the decoded bytes parse as JSON matching the argument pattern, for example a fetched packet's `format`). `capture` stores JSON-pointer values from a response for later steps. An object with a `$` directive is that directive alone, so `check-fixtures` refuses one that also has other members. Values the runner sends may use `{"$var": name}`, `{"$unique": prefix}` and `{"$repeat": [text, count]}`, which expands to `text` repeated `count` times. Every frame a participant sends must fit the caller's receive limit: 1 MiB, or the `receive_limits.max_frame_bytes` of the session's successful negotiation.
...
| `context.knowledge_provider` | `{ provider_id, socket, credential, grant? }`: a separately running knowledge provider where the context provider reads claims for `context.claims` sections, as an ordinary authenticated reader under `grant` (CONTEXT §14). Without it, claims naming this provider are read in its own knowledge store. Script `section` steps may carry `claim`, the exact claim revision reference the section presents. |
...
| `evidence_store` | Scripted evidence store for `evidence/1` (decision 007): `corrupt` and `unavailable` list artifact IDs whose stored bytes fail integrity or cannot be served; `staging_timeout_seconds` abandons staged uploads with reason `staging_expired`; `deletion_delay_seconds` confirms physical deletion that long after a purge request, on the controlled clock; the adversarial `serve_altered_bytes` makes fetch return those artifacts' bytes altered while keeping the sealed digest and `available`, so a reader's own digest check is tested. Holds and purges run through protocol operations. Participants declare `claims.test_controls: ["evidence.store"]`. Normative only for the conformance tests that use it. |
| `knowledge` | Knowledge controls for `knowledge/1` (KNOWLEDGE §13, participants declare `knowledge.store`): `evaluator: { id, version, condition_kinds }` is the applicability evaluator pinned in evaluations (default `reference-conditions` version `1`, all three condition kinds); the adversarial `serve_altered_claims` lists claim IDs whose inspected record is altered while the reference and digest stay unchanged. |
| `verifier` | Scripted evaluator for `verification/1` (VERIFICATION §11, participants declare `verifier.script`): `evaluators: [ { id, version, status } ]` become capability predicates `verification.evaluator.<id>.v<version>`; `scripts` maps job IDs to one-key steps, `default_script` applies otherwise (absent: complete at once). Steps: `wait_until` (instant), `property` (`property_id`, `result`, `reason?`, recorded durably on the job), `observe` (`anchors`), `valid_until` (instant) and `complete`. Steps advance no later than the provider's next request. Restarting with different `evaluators` changes the capability snapshot (SCN-7). Contracts and receipts are read from and sealed in the provider's own evidence store. |
