...
> Fourteenth spec-only pass, a short realignment for M6. Read first: `git diff 6ed4727 a77219e -- docs/spec schemas conformance/README.md conformance/schemas docs/decisions docs/VERIFICATION.md` (CORE §3, §4.2, §4.3, §10 step 1; EXECUTION §1, §13.3; VERIFICATION §11; the two `core.feature_dependencies` schemas; the fixture schema's `requires_pinned`; the README's `build_pinned.py` row); `docs/work/release-0.1/M6.md` in full; the CMP-5 row of MATRIX; the current CORE §15.5 and §18.2, CONTEXT §1, EVIDENCE §1, KNOWLEDGE §1, VERIFICATION §1, §4 and §11. Not read: `conformance/reference/**`, `conformance/runner/src/**`, `conformance/crosscheck/**`, `conformance/thirdparty/**`, `conformance/scripts/**`, their history or diffs, or reference `mutants` lists. The runner was built at this base and used as a binary. The readings below were written before any fixture was opened.

#### Change list, from the documents

| # | Text | Before | Change |
|---|---|---|---|
| 1 | CORE §4.3, §3 item 1, §10 step 1: `core.feature_dependencies`, a base `core/1` query, allowed before negotiation | `method_not_found` | Payload `{}` (closed); result `{ dependencies }` built from one table that negotiation also uses: one `profile` entry per supported profile major other than `core` (describe order), naming `core/1` and the required Core features in describe feature order, then `feature` entries for supported features with an enforced dependency. No command record, event or session change. |
| 2 | CORE §4.2 "Dependencies", profile-triggered | `depends_on` pass, then the Core-feature pass | Same results, now driven by the table of change 1 |
| 3 | CORE §4.2 feature-triggered; EXECUTION §13.3 | No feature dependencies | A requested, supported feature whose dependency is not among the session's selectable features is not selected, with `{ profile, feature, reason: "dependency_not_selected" }`: required feature in a required profile refuses with `unsupported_required_feature`; required in an optional profile leaves the profile unselected (item in `unselected`); optional feature reported in `unselected`. The table holds `execution.claim_revalidation` → `execution/1` with `execution.context_revalidation`, applied only while the feature is supported (see HM6-UNSUPPORTED-TRIGGER). |
| 4 | VERIFICATION §11: a job stays `queued` while its script waits at a leading `wait_until` | Every job became `running` at its first tick | Leading `wait_until` steps are consumed while `queued`; the job becomes `running` when the first other step is due, then that step runs. Losing the evaluator during the wait completes the job from `queued` (VERIFICATION §4, HM5C-QUEUED-LOSS-EVENTS). |

**Baseline at this base** (the `a77219e` tree, before any change, runner output only): `run: 276 fixtures (1 fail, 244 pass, 26 skipped, 5 unsupported), 1 not passing`. The failure is `verification.reconnect-with-changed-evaluator-keeps-pinned-meaning` step 27 (`result/state: expected "queued", found "running"`), which is change 4. `core.feature-dependencies-match-negotiation` is `unsupported` (`participant does not claim feature execution.claim_revalidation`), `compat.feature-dependencies-older-provider-falls-back` is skipped (`fixture targets the pinned participant 0.1-m5`) and `socket.feature-dependencies-after-authentication` is skipped (Unix binding).

#### Readings of points the documents leave open

- **HM6-UNSUPPORTED-TRIGGER** (CORE §4.3 "What is listed"; EXECUTION §1 "Machine-readable dependencies"; MATRIX CMP-5 column A). EXECUTION §1 says the query reports `execution.claim_revalidation`'s dependency "as a `feature` entry", unconditionally. CORE §4.3 lists "exactly the dependencies negotiation enforces", entries only for supported profile majors, and EXECUTION §1 has a provider advertise only the features it implements. This provider does not implement `execution.claim_revalidation` (H.M5b coverage limits), so requesting it is `unknown_feature` as before, negotiation never enforces its dependency, and the query lists no `feature` entry. Read: EXECUTION §1's sentence describes a provider that supports the feature. The feature-triggered rule is still implemented from a table filtered by support, and checked by a probe that enables the feature in a scratch copy (`tests/probe_m6.py`). Consequence: on this participant the CMP-5 fixture `core.feature-dependencies-match-negotiation` is `unsupported`, so neither the query nor feature-triggered enforcement is fixture-checked here. Basis: spec text (CORE §4.3), own judgment on EXECUTION §1.
- **HM6-REFUSAL-CODE** (CORE §4.2). The code rule is keyed on reasons ("When items fail for different reasons, the code is chosen in this order"), but `dependency_not_selected` now maps to `unsupported_profile` for a profile-triggered item and to `unsupported_required_feature` for a feature-triggered one. Implemented: each refusal item gets a code by its kind (profile-triggered and unknown or unsupported profiles: `unsupported_profile`; `no_common_major`: `unsupported_version`; `unknown_feature` and feature-triggered: `unsupported_required_feature`), then the stated order picks the code. So a required profile refused both for a missing Core feature and for a feature-triggered item is `unsupported_profile`. Basis: spec text, with own judgment on the mixed case.
- **HM6-DEPENDENCY-STATE** (CORE §4.2 "applied after selection", §4.3 `requires`). "Not selected" is evaluated for feature-triggered dependencies against the requested and supported features of the dependency's profile (the selection before dependencies), removing features to a fixed point, and not against the final selection. Otherwise a required `execution/1` request missing `core.effects`, that also asks for both `execution.claim_revalidation` and `execution.context_revalidation`, would get a spurious feature item because `execution/1` itself was dropped. Items: a required feature's item is listed even when a profile-triggered item already dropped the profile (every unsatisfied item is listed, as `unknown_feature` items are beside Core items, G-NEG-EXEC-ITEMS); an optional feature's item only while its profile stays selected; a profile dropped by a feature-triggered item still gets its profile-triggered items; a profile refused for an unknown required feature is not evaluated for feature-triggered dependencies. (Revised before any fixture was opened: the first committed wording dropped the required feature's item for a profile already dropped, which `tests/probe_m6.py` showed contradicts "lists every unsatisfied item".) Basis: own judgment.
- **HM6-ITEM-ORDER** (CORE §4.2). No order is given for items. Kept: selection items (`unknown_profile`, `declared_unsupported`, `no_common_major`, `unknown_feature`) in request order with Core first, then profile-triggered items per profile in describe order (as before this pass), then feature-triggered items per profile in request order, each profile's features in the order the caller listed them (required, then optional), then the `unknown_feature` items of optional features for profiles still selected. Basis: own judgment (profile before feature follows the §4.2 bullet order and the §4.3 entry order).
- **HM6-UNSELECTED-PROFILE-ITEMS** (CORE §4.2). When a required feature's dependency leaves an optional profile unselected, the `unselected` items for that profile are the feature-triggered item(s) and any profile-triggered items. Items of its optional features (unknown or dependency-missing) are dropped, as `unknown_feature` misses already were for an unselected profile. Basis: own judgment, consistent with the existing handling.
- **HM6-REQUIRED-TWICE** (CORE §4.2). A feature listed in both `required_features` and `optional_features` is treated as required for the feature-triggered rule. Basis: own judgment.
- **HM6-REQUIRES-ORDER** (CORE §4.3). The order of `requires` entries, and of `features` inside one, is not stated. One `requires` entry per dependency profile major; `features` in that profile's describe feature order (`core.events`, `core.capabilities`, `core.effects`), which is also the order of M6.md's example. The `core-test/1` entry is `requires: [ { profile: "core", major: 1, features: [] } ]`, and `core` itself has no entry. Basis: spec text for the entries, own judgment for the order.
- **HM6-QUERY-PROTECTION** (CORE §15.5 "Which operations are protected"). The unprotected list was not extended with `core.feature_dependencies`. Read as unprotected, like `core.describe` ("beside `core.describe`", read-only, manifest-only), so a `grant` field is validated where the envelope allows it and never evaluated. Basis: own judgment; suggested: add the query to §15.5's list.
- **HM6-QUERY-ENVELOPE** (CORE §10 steps 2–3). The query follows the Core query path: `method_not_found` never; limits; closed envelope with an empty closed payload (`invalid_envelope` for any payload member); a `requires` entry before negotiation is `unsupported_required_feature` (D-REQUIRES-PRENEG); after negotiation it still answers (no `already_negotiated`). Basis: spec text.
- **HM6-PREAUTH** (CORE §3 item 1, §4.3, §18.2). §3 and §10 list the query beside `core.describe` before negotiation; §4.3 and §18.2 still require authentication first on a shared transport, where only `core.describe` and `core.authenticate` are allowed. Not a contradiction, but §3 item 1 reads as if the query were allowed wherever `core.describe` is. Not applicable to stdio. Basis: spec text.
- **HM6-BOUNDED** (CORE §4.3 "Bounded"). The result is a constant of the manifest (well under 1 MiB) and does not depend on the negotiated receive limit. Basis: spec text.
- **HM6-QUEUED-WAIT-STEPS** (VERIFICATION §11). While `queued`, each leading `wait_until` whose instant has passed is consumed without an event; the `verification.job.changed { state: "running" }` event is appended when the first other step is due (in its own transaction, as before), and that step then runs. `observed_from` is the instant the job became `running`. Basis: spec text, own judgment for the event granularity.
