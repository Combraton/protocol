...
It was written in four spec-only passes:
...
The later passes also read the resolution record [M2-DIVERGENCES](../../../docs/work/release-0.1/M2-DIVERGENCES.md). The third and fourth passes read the documents before the fixtures.
...
- `docs/work/release-0.1/M2-DIVERGENCES.md` (second to fourth passes)
...
In the first two passes the fixtures were read **before** the code was written, so some choices on points the prose leaves open were informed by what the fixtures expect. [DIVERGENCES.md](DIVERGENCES.md) marks each such choice. The third and fourth passes wrote their change lists from the documents first and read the fixtures afterwards (sections F and G). The fourth pass committed its implementation before opening any `execution/`, `socket/` or M3 Core fixture.
...
Two genuine misreadings were fixed after the first run (DIVERGENCES G-EXIT-RUNTIME, G-COALESCE-ADJACENT). The 14 remaining failures are all Execution fixtures that expect names or behavior the documents do not state, or in one case contradict (DIVERGENCES G.2); they were left failing, so `run` exits 1 for this participant. The two clock-file fixtures that were coverage limits before this pass, `core.events.subscription-ends-at-grant-expiry` and `core.grants.test-clock-never-moves-backward`, now run and pass. The four backpressure fixtures remain `unsupported`, and the 12 `socket.*` fixtures are skipped.
...
- Execution: one scripted host. The harness is the launch configuration's script; there is no real adapter, no inline brief and no barrier or signal. A scripted crash exits with status 1 between requests.
