...
pub const ALL: &[(&str, &str)] = &[
    ...
    (
        "uncheckable-item-accepted",
        "accepts an item whose check kind is not checkable",
    ),
    (
        "dirty-basis-complete-accepted",
        "accepts a commit-only basis for a dirty workspace as complete",
    ),
    (
        "limits-collapsed",
        "charges investigation against output capacity and reports budget exhaustion as the deadline",
    ),
    (
        "packet-bytes-regenerated",
        "serves packet excerpts from regenerated bytes instead of the sealed artifact",
    ),
    (
        "excerpt-carries-digest",
        "labels a packet excerpt with the artifact digest",
    ),
    (
        "revision-remapped",
        "maps an earlier packet revision to the latest artifact",
    ),
    (
        "global-coverage-cursor",
        "merges per-producer coverage into one global frontier",
    ),
    (
        "required-downgraded-to-advisory",
        "downgrades a missing required item to advisory at the deadline",
    ),
    (
        "missing-required-satisfied",
        "reports a missing required item satisfied",
    ),
    (
        "mandatory-items-dropped",
        "drops mandatory items that do not fit instead of refusing",
    ),
    (
        "transition-as-advisory",
        "accepts required_before_transition without negotiation and treats it as advisory",
    ),
    (
        "cancel-ends-job",
        "ends the shared job when one subscriber cancels",
    ),
    (
        "expand-ignores-grant",
        "expands citations the principal may not read",
    ),
    (
        "packet-mutated-in-place",
        "replaces the published packet revision instead of adding one",
    ),
    (
        "old-revision-relabeled-current",
        "reports superseded packet revisions as current",
    ),
    (
        "stale-derivation-current",
        "keeps a derivation from a corrected binding current",
    ),
    (
        "unobserved-frontier-claimed",
        "claims coverage frontiers beyond what was ingested",
    ),
    (
        "unlabeled-section",
        "publishes packet sections without labels",
    ),
    (
        "authority-revision-dropped",
