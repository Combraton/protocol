...
- **Protocol use:**
  ...
  - Each session authenticates, calls `core.feature_dependencies` (CORE §4.3), then negotiates.
...
1. **Not held.** The packet bytes cannot be fetched at the artifact's provider, or the SHA-256 digest of the exact bytes received differs from the reference's digest: state `unsatisfied`.
2. **Facts unreadable.** The packet's result facts cannot be read at the Context provider (`context.packet.inspect` under the context grant), or the facts name a different artifact or digest for that revision: state `unknown`.
...
**Mutant `ignores-required-boundary`.** This deliberately broken kernel still evaluates and publishes honest check records, but it dispatches every due item whatever its state. It exists only so fixtures can prove they detect a kernel that ignores the boundary.
