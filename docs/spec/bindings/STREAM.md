...
2. **Content.** A frame's bytes MUST be valid UTF-8 forming one JSON text that conforms to I-JSON ([RFC 7493](https://www.rfc-editor.org/rfc/rfc7493)):
...
5. **Frame limit.** Each receiver applies a frame limit counted in bytes, excluding the terminator.
   ...
   - After negotiation, each side's limit is the value it advertised: the provider in its negotiation result, the caller in its negotiation request's `receive_limits`. The value is never below 1 MiB.
...
These failures mean the stream can no longer be trusted. The receiver sends one error response with `"id": null`, flushes for at most one second, and closes the connection:
...
- **Request IDs.** A request `id` is a string of 1–128 bytes or an integer within ±(2^53 − 1). A null or fractional `id` is `invalid_request`. The `id` matches a response to a request on one connection only. It MUST NOT be used for deduplication, and a caller MAY reuse it after the response arrives.
...
- **Notifications from a caller** — requests without `id` — are not used in 0.1. A provider MUST NOT process one and MUST NOT reply. It MAY log it. A command sent as a notification therefore has no effect; the caller learns nothing, as JSON-RPC requires.
...
- **Exit.** On end of standard input, after the drain period, the provider exits. Durable state is kept; a new process is a new session over the same state.
