...
- **Conformance conventions** (as EXECUTION §15.1: normative only for the conformance provider, never for callers): a job started by request `r` has ID `r`; packet revision `n` of request `r` is sealed as artifact `packet.<r>.<n>`; the compiler is `combraton-reference-context`.
