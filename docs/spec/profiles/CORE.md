...
- **Backpressure (proposed M3 draft, feature `core.events.backpressure`).** Semantic events are never skipped. A consumer that stops reading loses its connection, not events:
  ...
  - **Room, then too slow.** The provider waits at most `backpressure_notice_ms` for the consumer to read what is pending. A consumer that makes no room within that time is too slow. The same rule applies when a response would exceed the bound.
  - **Ending notice, attempted and bounded.** If the session negotiated `core.events.backpressure`, the provider attempts one final `core.events.notify` per affected subscription with `"items": []` and `"ended": { "reason": "consumer_too_slow" }`. It spends at most its declared notice write bound on these frames, and never waits indefinitely for a client that does not read.
  - **Closure.** Whether or not the notice was written, the provider then closes the connection and writes nothing more on it. Every subscription on it ends.
  ...
  - **Declared bounds.** Negotiating the feature adds `max_pending_notification_bytes` and `backpressure_notice_ms` to the result's `limits`. Without the feature, those members are absent. Sessions that did not negotiate the feature are still bounded and closed; they only lack the declaration and the notice.
  - **Conformance.** Fixtures pause the runner's reading and synchronize on implementation-specific signals (decision 007): `backpressure.limit.reached` when the consumer is declared too slow, and `backpressure.connection.closed` after closure. A participant that does not declare them reports these fixtures as `unsupported` coverage limits.
