...
| G8-INSPECT-OBLIGATIONS | **spec:** EXECUTION §4 — `execution.inspect` lists obligations still waiting (`open` or `overdue`); closed ones are read through `core.effects.get`. **reference:** it listed every obligation and now filters. **fixture:** `execution.recovery-revalidates-before-dispatch` v2 reads the satisfied obligation through `core.effects.get`. The independent implementation already behaved this way. |
