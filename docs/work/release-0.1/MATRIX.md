...
- **F:** fault injection through the test-control channel.
...
| REL-8 | Scoped conformance claims | P last §; REL | The result manifest names implementation and version, suite/schema/fixture versions, claimed profiles and features, and per-case outcome (passed, failed, unsupported, not run). | M1 |
...
| ID | Requirement | Source | Acceptance evidence | M |
|---|---|---|---|---|
| EXE-1 | Submit returns an execution reference. A retry is a new execution linked to its predecessor. | P §3, §8; PIO §4; PIO-I §2 | R: retry linkage. M: reuses the execution ID for a retry. | M3 |
| EXE-2 | Admission (queued, admitted, refused, with reason); a required restriction that cannot be enforced is refused with an alternative | PIO §5, §7 | A: required enforcement above the advertised level is refused. M: admits with cooperative enforcement. | M3 |
| EXE-3 | Delivery axis and proof classes; ambiguity preserved, later reconciliation appended | PIO §5 | F: crash after write. Ambiguous, then reconciled, with both events retained. M: overwrites the ambiguity, or labels `bytes_written` as acknowledged. | M3 |
| EXE-4 | Runtime axis includes `requires_action` (action ID, owner) and `unknown` | P §5; PIO §5 | R plus schema. M: represents `requires_action` as a progress level. | M3 |
| EXE-5 | Result, exit and evaluation are separate; evaluation is never inferred from exit | PIO §5 | M: sets evaluation from exit 0. | M3 |
| EXE-6 | Immutable completion receipt: matching duplicate returns it, conflict is rejected, an old attempt cannot finalize its successor | PIO §5; PIO-I §6 | A: conflicting completion. F: late old-attempt result. M: last write wins. | M3 |
| EXE-7 | Inspect and watch execution facts with cursors | P §8; PIO §10 | R, reusing OBS fixtures on execution streams. | M3 |
| EXE-8 | Cancel returns a request receipt; `cancel_requested` is not `cancelled`; lost cancel acknowledgment is recoverable | P §5, §13; PIO §8 | F: drop the cancel acknowledgment, then retransmit and get the same receipt. M: reports cancelled on request. | M3 |
| EXE-9 | Steering: recorded, native-acknowledged and observed behavior are separate; unsupported is explicit | P §12; PIO §7 | A: steer against a no-steering capability gives an explicit limitation. M: claims delivery. | M3 |
| EXE-10 | Reconcile by command or delivery ID returns scoped observations and never resubmits | P §8; PIO §8 | F: lost submit acknowledgment, then reconcile; exactly one execution. M: resubmits. | M3 |
| EXE-11 | Capability predicates with adapter/harness/protocol versions, probe time, limits and enforcement level; unknown is not supported | PIO §3; PIO-I §3 | Schema. A: a missing predicate treated as unknown. M: defaults to supported. | M3 |
| EXE-12 | Discovery facts kept separate: detected, recognized, version supported, authentication known or unknown, reachable, last verified | PIO-C §3 | Schema and R. A: detected but authentication unknown is not offered as usable. Proposed as an optional feature. | M3 |
| EXE-13 | One mutating controller lease with epoch; stale controller refused; read-only observers coexist | PIO §6; P §13 | A: two controllers claim one host. F: reconnect increments the epoch. M: accepts the stale controller. | M3 |
| EXE-14 | A native action request belongs to its execution; answering one authorizes nothing else | PIO-C §4 | A: answer execution A's action ID in execution B. M: global action namespace. | M3 |
| EXE-15 | Workspace lease descriptor and checkpoint coverage (dirty and untracked state); agent-reported SHAs are annotations | PIO §7, §10 | Schema. A: checkpoint with incomplete coverage declared. M: agent SHA reported as receipt. | M3 |
| EXE-16 | Usage basis (observed, estimated, unknown, enforced bound); unresolved liability after timeout; unenforceable hard ceilings refused | PIO §9; PIO-I §5 | F: timeout with unknown usage keeps the liability. A: hard dollar ceiling without enforcement is refused. M: refunds on timeout. | M3 |
| EXE-17 | Context binding at submit; required-before-start without a satisfied binding is not admitted as ready | PIO context §; CBR-P §4, §6 | A: required-before-start with an unmet item. R: advisory proceeds with a visible gap. | M3 |
| EXE-18 | Packet delivery observation: exact digests, target, boundary, outcome including late, unavailable and unknown | CBR-P §6, §7; PIO context § | F: update arrives after the dependent boundary and is recorded late. M: no late state. | M3 |
| EXE-19 | Queue, delivery, execution-deadline, inactivity and reconciliation timeouts are distinct | PIO §9 | Schema plus R. A: execution deadline expiry does not claim remote work ended. | M3 |
| EXE-20 | Opaque caller correlation without a Combraton model | PIO §10 | R: arbitrary correlation round-trips. | M3 |
| EXE-21 | Outcome references exported artifacts and evidence | P §9; PIO §10 | X with Evidence in M4. | M3–M4 |
| EXE-22 | Detach and close are not cancellation; a restarted client queries rather than replays | PIO-C §2 | F: client disconnect; execution continues and reconnect observes it. | M3 |
| EXE-23 | Fresh continuation is labeled differently from native resume | PIO §6 | A: resume without native support yields a labeled fresh continuation. | M3 |
...
| SCN-2 | Lost submit acknowledgment creates no duplicate | PLAN §13; PIO §8 | F plus EXE-10. | M3 |
| SCN-3 | Delayed old-attempt result after replacement | P §13; PIO §8 | F plus EXE-6. | M3 |
...
| SCN-10 | Crash between journal and dispatch | DEV §8; PIO §4 | F plus EXE-3. | M3 |
